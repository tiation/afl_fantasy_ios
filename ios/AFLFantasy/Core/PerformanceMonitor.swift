        ...
        memoryTimer = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: true) { [weak self] _ in
            ...
            Task { @MainActor in
                ...
                if self.memoryUsage > 220 {
                    self.logger.warning("⚠️ Memory usage exceeded budget: \(self.memoryUsage, format: .fixed(precision: 1))MB")
                }
            }
        }
...
extension URLSession {
    ...
    static let aflOptimized: URLSession = {
        ...
        config.urlCache = URLCache(memoryCapacity: 10 * 1024 * 1024, // 10MB
