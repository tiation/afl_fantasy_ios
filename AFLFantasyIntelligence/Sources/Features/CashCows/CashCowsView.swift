...
struct CashCowRowView: View {
    ...
    var body: some View {
        ...
        .dsAccessibility(
            label: "\(cashCow.playerName), current price \(cashCow.currentPrice), projected \(cashCow.projectedPrice), cash generation \(cashCow.cashGenerated), recommendation \(cashCow.recommendation)",
            traits: .button
        )
    }
