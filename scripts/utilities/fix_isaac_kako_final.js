/**
 * Fix Isaac Kako Data with Updated Information - Final Version
 * 
...
function updateKakoData(teamData) {
  ...
  // Search for Kako with flexible pattern matching
  let updated = false;
  
  // Process all positions including bench
  const positions = ['defenders', 'midfielders', 'rucks', 'forwards'];
  
  // Search main positions first
    ...
    if (Array.isArray(teamData[position])) {
      for (let i = 0; i < teamData[position].length; i++) {
        ...
        if (player && player.name) {
          ...
          // Check for various versions of the name
          if (playerName.includes('kako') || 
              playerName.includes('isaac k')) {
...
async function fixKakoData() {
  try {
    ...
    // Create backup
    const backupPath = `${TEAM_FILE_PATH}.kako_ultrafinal_backup`;
