...
class AdvancedAnalyticsService: ObservableObject {
    ...
    func analyzeCashGeneration(for players: [Player]) -> [CashCowAnalysis] {
        let cashCows = players.filter { $0.isCashCow }
        ...
        return cashCows.map { player in
            ...
            let recommendation = determineCashCowRecommendation(player: player, sellWindow: sellWindow, holdRisk: holdRisk)
    ...
    private func predictThreeRoundPriceChange(player: Player) -> PriceChangePrediction {
        let totalScoreDifference = player.threeRoundProjection.reduce(0.0) { acc, projection in
            return acc + (projection.projectedScore - Double(player.breakeven))
        }
    ...
    private func predictSeasonEndPrice(player: Player) -> Int {
        ...
        return max(player.currentPrice + Int(totalPriceChange), 100000) // Minimum price floor
    }
    ...
    private func calculatePriceChangeFromScoreDifference(scoreDifference: Double, currentPrice: Int) -> Double {
        ...
        let priceMultiplier = Double(currentPrice) / 500000.0 // Normalize around $500k
    ...
    private func generatePriceChangeReasoning(scoreDifference: Double, player: Player) -> String {
        if scoreDifference > 15 {
            return "Projected to score \(Int(scoreDifference)) above breakeven (\(player.breakeven))"
        } else if scoreDifference < -15 {
            return "Projected to score \(Int(abs(scoreDifference))) below breakeven (\(player.breakeven))"
        } else {
            return "Projected score close to breakeven - minimal price movement expected"
        }
    }
    ...
    private func generatePriceHistory(player: Player) -> [PriceHistoryPoint] {
        ...
        for week in 1...10 {
            ...
            currentPrice = max(currentPrice + randomChange, 100000)
    ...
    private func determineBuySellTiming(
        ...
        if futureOutlook.expectedGain > 50000 && valueAssessment == .undervalued {
            return (.buy, 0.85, "Strong price rise expected (+$\(futureOutlook.expectedGain/1000)k) from undervalued position")
    ...
    private func determineReliabilityGrade(consistency: Double) -> ReliabilityGrade {
        ...
        case 70..<80: return .veryGood
        case 60..<70: return .good
        case 50..<60: return .average
        case 40..<50: return .poor
    ...
    private func calculateHistoryRisk(player: Player) -> Double {
        ...
        let severityScore = recentInjuries.reduce(0.0) { acc, injury in
            return acc + (Double(injury.weeksOut) / 22.0) // Normalize by season length
        }
    ...
    private func calculateAgeRisk(player: Player) -> Double {
        ...
        let estimatedAge = player.currentPrice > 600000 ? 28.0 : 22.0 // Premium vs rookie assumption
    ...
    private func calculatePositionRisk(player: Player) -> Double {
        ...
        case .midfielder: return 0.6 // Highest running load
        case .ruck: return 0.7 // High contact
...
enum TimingRecommendation: String, CaseIterable, Codable {
    ...
    var color: Color {
        switch self {
        case .buy, .buyDip: return .green
        case .sell, .sellHigh: return .red
