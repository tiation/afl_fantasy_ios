...
class KeychainManager: ObservableObject {
    ...
    private enum Keys {
        ...
        // Enhanced personalization keys
        static let avatarURL = "avatar_url"
        static let userBio = "user_bio"
        static let favoriteTeamId = "favorite_team_id"
        static let detailedNotificationPrefs = "detailed_notification_prefs"
        static let themePreference = "theme_preference"
        static let aiPersonalizationSettings = "ai_personalization_settings"
    }
    ...
    // MARK: - Enhanced Personalization Methods
    
    /// Store user's avatar URL
    func storeAvatarURL(_ url: String) {
        store(key: Keys.avatarURL, value: url)
    }
    
    /// Get user's avatar URL
    func getAvatarURL() -> String? {
        retrieve(key: Keys.avatarURL)
    }
    
    /// Store user's bio
    func storeBio(_ bio: String) {
        store(key: Keys.userBio, value: bio)
    }
    
    /// Get user's bio
    func getBio() -> String? {
        retrieve(key: Keys.userBio)
    }
    
    /// Store user's favorite team (enhanced - team ID)
    func storeFavoriteTeamId(_ teamId: String) {
        store(key: Keys.favoriteTeamId, value: teamId)
    }
    
    /// Get user's favorite team ID
    func getFavoriteTeamId() -> String? {
        retrieve(key: Keys.favoriteTeamId)
    }
    
    /// Store detailed notification preferences as JSON string
    func storeDetailedNotificationPreferences(_ preferences: DetailedNotificationPreferences) {
        if let data = try? JSONEncoder().encode(preferences),
           let jsonString = String(data: data, encoding: .utf8) {
            store(key: Keys.detailedNotificationPrefs, value: jsonString)
        }
    }
    
    /// Get detailed notification preferences
    func getDetailedNotificationPreferences() -> DetailedNotificationPreferences {
        guard let jsonString = retrieve(key: Keys.detailedNotificationPrefs),
              let data = jsonString.data(using: .utf8),
              let preferences = try? JSONDecoder().decode(DetailedNotificationPreferences.self, from: data)
        else {
            return DetailedNotificationPreferences.defaultPreferences
        }
        return preferences
    }
    
    /// Store theme preference as JSON string
    func storeThemePreference(_ preference: ThemePreference) {
        if let data = try? JSONEncoder().encode(preference),
           let jsonString = String(data: data, encoding: .utf8) {
            store(key: Keys.themePreference, value: jsonString)
        }
    }
    
    /// Get theme preference
    func getThemePreference() -> ThemePreference {
        guard let jsonString = retrieve(key: Keys.themePreference),
              let data = jsonString.data(using: .utf8),
    ...
    /// Store AI personalization settings as JSON string
    func storeAIPersonalizationSettings(_ settings: AIPersonalizationSettings) {
        if let data = try? JSONEncoder().encode(settings),
           let jsonString = String(data: data, encoding: .utf8) {
            store(key: Keys.aiPersonalizationSettings, value: jsonString)
        }
    }
    
    /// Get AI personalization settings
    func getAIPersonalizationSettings() -> AIPersonalizationSettings {
        guard let jsonString = retrieve(key: Keys.aiPersonalizationSettings),
              let data = jsonString.data(using: .utf8),
              let settings = try? JSONDecoder().decode(AIPersonalizationSettings.self, from: data)
        else {
            return AIPersonalizationSettings.defaultSettings
        }
        return settings
    }
    
    /// Enhanced complete user profile with all personalization data
    func completeEnhancedUserProfile(
        name: String,
        ...
        bio: String? = nil,
        detailedNotificationPrefs: DetailedNotificationPreferences? = nil,
        themePreference: ThemePreference? = nil,
        aiSettings: AIPersonalizationSettings? = nil
    ) {
        // Store basic user preferences
        storeUserName(name)
        storeFavoriteTeamId(favoriteTeamId)
        
        // Store AFL Fantasy credentials
        storeAFLCredentials(teamId: teamId, sessionCookie: sessionCookie, apiToken: apiToken)
        
        // Store enhanced personalization data
        ...
        storeDetailedNotificationPreferences(detailedNotificationPrefs ?? DetailedNotificationPreferences.defaultPreferences)
        storeThemePreference(themePreference ?? ThemePreference.defaultPreference)
        storeAIPersonalizationSettings(aiSettings ?? AIPersonalizationSettings.defaultSettings)
        
        // Mark onboarding as completed
        setOnboardingCompleted(true)
    }
    
    /// Check if enhanced user profile is complete
    func hasCompleteProfile() -> Bool {
        return hasCompletedOnboarding() &&
               hasAFLCredentials() &&
               getUserName() != nil &&
               getFavoriteTeamId() != nil
    }
    
    /// Clear all user data (for logout or reset) - enhanced version
    func clearAllUserData() {
        ...
        // Clear enhanced personalization data
        delete(key: Keys.avatarURL)
        delete(key: Keys.userBio)
        delete(key: Keys.favoriteTeamId)
        delete(key: Keys.detailedNotificationPrefs)
        delete(key: Keys.themePreference)
        delete(key: Keys.aiPersonalizationSettings)
    }
