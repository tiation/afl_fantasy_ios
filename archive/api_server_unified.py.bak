#!/usr/bin/env python3
"""
AFL Fantasy API Server with Integrated WebSocket Support
Unified server with REST API and WebSocket real-time updates
"""
...
async def websocket_handler(websocket, path):
    """Handle WebSocket connections at /ws/live"""
    # Register client
...
def simulate_live_updates():
    ...
    while True:
        ...
        if random.random() > 0.7:
            ...
            # Get random player from cache for realistic alerts
            if players_cache:
                random_player_id = random.choice(list(players_cache.keys()))
                player_info = extract_player_info(players_cache[random_player_id])
                player_name = player_info.get("name", "Unknown Player")
                
                if alert_type == "PRICE_CHANGE":
                    price_change = random.choice([5000, 7500, 10000, 12500, 15000])
                    direction = random.choice(["increased", "decreased"])
                    loop.run_until_complete(broadcast_alert(
                        "PRICE_CHANGE",
                        "Price Movement Alert",
                        f"{player_name} has {direction} by ${price_change:,}",
                        random_player_id
                    ))
                    ...
                    loop.run_until_complete(broadcast_alert(
                        "INJURY",
                        "Injury Update",
                        f"{player_name} {status} for next match",
                        random_player_id
                    ))
                    ...
                    loop.run_until_complete(broadcast_alert(
                        "LATE_OUT",
                        "Late Out Alert",
                        f"{player_name} withdrawn from team",
                        random_player_id
                    ))
                else:
                    role = random.choice(["midfield", "forward", "defense", "ruck"])
                    loop.run_until_complete(broadcast_alert(
                        "ROLE_CHANGE",
                        "Role Change",
                        f"{player_name} moved to {role}",
                        random_player_id
                    ))
...
def start_websocket_server():
    ...
    async def serve():
        async with websockets.serve(websocket_handler, "0.0.0.0", ws_port):
            log_info(f"🔌 WebSocket server started on ws://localhost:{ws_port}/ws/live")
...
def send_live_alert():
    ...
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(broadcast_alert(alert_type, title, message, player_id))
    
    log_info(f"Sent alert '{title}' to {len(websocket_clients)} clients")
...
if __name__ == '__main__':
    ...
    if players_cache:
        ...
        print("")
        print("📝 Test custom alert with curl:")
        print(f'   curl -X POST http://localhost:{port}/api/live/alert \\')
        print('        -H "Content-Type: application/json" \\')
        print('        -d \'{"type":"INJURY","title":"Test Alert","message":"Player injured"}\'')
        print("="*60)
