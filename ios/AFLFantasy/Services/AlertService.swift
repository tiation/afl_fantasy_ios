...
class AlertService: ObservableObject {
    ...
    @Published var alertPreferences: AlertPreferences = .init()
    ...
    private func calculatePriceDropRiskFactors(player: Player) -> [Double] {
        ...
        let breakevenRisk = player.breakeven > 0 && Double(player.breakeven) > player.nextRoundProjection
            .projectedScore * 1.1 ? 0.3 : 0.0
        ...
        let injuryRisk = player.injuryRisk.riskLevel == .high ? 0.2 : player.injuryRisk
            .riskLevel == .moderate ? 0.1 : 0.0
        factors.append(injuryRisk)

        // Factor 4: Venue disadvantage
        let venueRisk = player.venuePerformance.first { $0.venueName == player.nextRoundProjection.venue }?
            .bias ?? 0.0
        factors.append(venueRisk < -3 ? 0.15 : 0.0)

        // Factor 5: Hard opponent matchup
        let opponentRisk = player.opponentPerformance
            .first { $0.opponentTeam == player.nextRoundProjection.opponent }?.dvpRanking ?? 10
    ...
    private func analyzeBreakevenCliff(player: Player, round: Int) -> AlertFlag? {
        ...
        guard player.breakeven > 0, player.currentPrice > 400_000 else { return nil }
    ...
    private func analyzeCashCowSellSignal(player: Player, round: Int) -> AlertFlag? {
        guard player.isCashCow, player.cashGenerated > 100_000 else { return nil }
    ...
    private func calculateCashCowSellFactors(player: Player) -> [Double] {
        ...
        factors.append(player.cashGenerated > 150_000 ? 1.0 : player.cashGenerated > 100_000 ? 0.7 : 0.3)
        ...
        factors.append(player.currentPrice + player.cashGenerated > 550_000 ? 0.9 : 0.3)

        // Factor 4: Future fixture difficulty
        let upcomingDifficulty = player.threeRoundProjection.reduce(0.0) { acc, projection in
            // Simplified difficulty calculation
            acc + (projection.projectedScore < player.averageScore ? 1.0 : 0.0)
        } / 3.0
    ...
    private func analyzeContractYearImpact(player: Player) -> AlertFlag? {
        ...
        let existingAlert = alertHistory.contains { alert in
            alert.type == .captainRecommendation && alert.title.contains(player.name) && alert.message
                .contains("contract")
        }
    ...
    private func analyzePremiumBreakout(player: Player) -> AlertFlag? {
        guard player.seasonProjection.premiumPotential > 0.8, player.currentPrice < 600_000 else { return nil }

        let recentTrend = player.seasonalTrend.trendDirection == .improving
        let valuePrice = player.currentPrice < 500_000
    ...
    private func generateByeRoundAlerts(players: [Player], currentRound: Int) -> [AlertFlag] {
        ...
        for round in (currentRound + 1) ... (currentRound + 3) {
            let playersOnBye = players.filter { player in
                ...
                player.teamId % 6 == round % 6 // Simplified bye logic
            }
    ...
    func shouldShowAlert(_ alert: AlertFlag) -> Bool {
        ...
        case .priceRise, .priceDrop:
            alertPreferences.priceChangeAlerts
        case .injuryRisk:
            alertPreferences.injuryAlerts
        case .breakEvenCliff:
            alertPreferences.breakevenAlerts
        case .cashCowSell:
            alertPreferences.cashCowAlerts
        case .tradeOpportunity:
            alertPreferences.tradeOpportunityAlerts
        case .captainRecommendation:
            alertPreferences.captainAlerts
        case .byeRoundWarning:
            alertPreferences.byeRoundAlerts
        case .roleChange:
            alertPreferences.roleChangeAlerts
        case .weatherRisk:
            alertPreferences.weatherAlerts
        }
    }
