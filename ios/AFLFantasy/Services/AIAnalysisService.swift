...
class AIAnalysisService: ObservableObject {
    ...
    private func findUpgradeOpportunities(
        ...
        for weakPlayer in weakestPlayers {
            ...
            if let bestUpgrade = samePositionPlayers.max(by: { $0.seasonProjection.averageProjectedScore < $1.seasonProjection.averageProjectedScore }) {
                ...
                let analysis = TradeAnalysis(
                    ...
                    nextRoundImpact: TradeImpact(
                        pointsGained: bestUpgrade.nextRoundProjection.projectedScore - weakPlayer.nextRoundProjection.projectedScore,
                        priceGained: bestUpgrade.nextRoundProjection.priceChange - weakPlayer.nextRoundProjection.priceChange,
    ...
    private func findCashCowOpportunities(
        ...
        return topCashCows.compactMap { cashCow in
            ...
            guard let tradeOut = benchPlayers.min(by: { $0.seasonProjection.averageProjectedScore < $1.seasonProjection.averageProjectedScore }) else { return nil }
            ...
            let analysis = TradeAnalysis(
                ...
                seasonImpact: TradeImpact(pointsGained: 120, priceGained: cashCow.seasonProjection.totalPriceRise, riskAdjustedValue: 0.75),
                risks: [TradeRisk(risk: "May plateau early", probability: 0.3, impact: .medium)],
                opportunities: [TradeOpportunity(opportunity: "Major price rises", likelihood: 0.8, benefit: "$\(cashCow.seasonProjection.totalPriceRise/1000)k gain")],
                recommendation: .buy
            )
    ...
    private func findCorrectionTrades(
        ...
        return fallingPremiums.compactMap { fallingPlayer in
            ...
            guard let bestAlternative = alternatives.max(by: { $0.seasonProjection.averageProjectedScore < $1.seasonProjection.averageProjectedScore }) else { return nil }
            ...
            let analysis = TradeAnalysis(
                ...
                seasonImpact: TradeImpact(pointsGained: 150, priceGained: 100000, riskAdjustedValue: 0.8),
                risks: [TradeRisk(risk: "Original player may bounce back", probability: 0.25, impact: .medium)],
                opportunities: [TradeOpportunity(opportunity: "Avoid further price drops", likelihood: 0.75, benefit: "Capital preservation")],
                recommendation: .sell
            )
    ...
    private func calculateTradeScore(from playerOut: Player, to playerIn: Player) -> Double {
        let pointsImprovement = (playerIn.seasonProjection.averageProjectedScore - playerOut.seasonProjection.averageProjectedScore) * 5
        let priceImprovement = Double(playerIn.seasonProjection.totalPriceRise - playerOut.seasonProjection.totalPriceRise) / 10000
    ...
    private func calculateSeasonImpact(from playerOut: Player, to playerIn: Player) -> TradeImpact {
        return TradeImpact(
            pointsGained: playerIn.seasonProjection.averageProjectedScore - playerOut.seasonProjection.averageProjectedScore,
    ...
    private func determineTradeRecommendation(analysis tradeScore: Double) -> TradeRecommendation {
        switch tradeScore {
        case 90...: return .strongBuy
        case 75..<90: return .buy
        case 50..<75: return .hold
        case 25..<50: return .sell
    ...
    private func calculateStructureGrade(
        ...
        let allocations = [defAllocation.allocation, midAllocation.allocation, rucAllocation.allocation, fwdAllocation.allocation]
        ...
        // Bench strength
        let avgBenchStrength = (defAllocation.benchStrength + midAllocation.benchStrength + rucAllocation.benchStrength + fwdAllocation.benchStrength) / 4
        if avgBenchStrength >= 60 { score += 25 }
        else if avgBenchStrength >= 45 { score += 20 }
        else if avgBenchStrength >= 30 { score += 10 }
        
        // Premium coverage
        let avgValues = [defAllocation.averageValue, midAllocation.averageValue, rucAllocation.averageValue, fwdAllocation.averageValue]
        let premiumCoverage = avgValues.filter { $0 >= 550000 }.count
    ...
    private func identifyStructureWeaknesses(
        ...
        [defAllocation, midAllocation, rucAllocation, fwdAllocation].forEach { allocation in
        ...
        let readyToSell = team.filter { $0.isCashCow && $0.cashGenerated > 150000 }
