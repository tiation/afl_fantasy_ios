...
struct DashboardView: View {
    ...
    @EnvironmentObject private var appViewModel: ContentViewModel
    ...
    private var quickActionsSection: some View {
        VStack(alignment: .leading, spacing: Theme.Spacing.m) {
            SectionHeader("Quick Actions", subtitle: "Navigate to key features")
            
            HStack(spacing: Theme.Spacing.m) {
                // Captain Suggestions Button
                Button {
                    withAnimation(.easeInOut(duration: 0.3)) {
                        appViewModel.selectedTab = .team
                    }
                } label: {
                    HStack(spacing: Theme.Spacing.xs) {
                ...
                // View Cash Cows Button
                Button {
                    withAnimation(.easeInOut(duration: 0.3)) {
                        appViewModel.selectedTab = .cashCows
                    }
                } label: {
                    HStack(spacing: Theme.Spacing.xs) {
            ...
            // Check Price Changes Button (full width)
            Button {
                withAnimation(.easeInOut(duration: 0.3)) {
                    appViewModel.selectedTab = .cashCows
                }
