...
class AFLExperienceManager: ObservableObject {
    ...
    private func playHapticExperience(_ type: AFLExperienceType) {
        ...
        case let .tradeComplete(success, _):
            hapticsManager.onTradeComplete(isGood: success)

        case .captainSelection:
            hapticsManager.onCaptainSelection()

        case let .scoreUpdate(oldScore, newScore, _):
            hapticsManager.onScoreUpdate(improvement: newScore - oldScore)

        case let .priceRise(_, change):
            hapticsManager.onPriceChange(change: change)

        case let .priceDrop(_, change):
            hapticsManager.onPriceChange(change: -abs(change))

        case .milestone:
            hapticsManager.onMilestoneReached()

        case .celebration:
            hapticsManager.playHaptic(.celebration)

        case .tradingDeadline:
            hapticsManager.onTradingDeadline()

        case .validationError, .insufficientFunds, .tradingClosed:
            hapticsManager.playHaptic(.error)

        case .goalScored:
            hapticsManager.playHaptic(.goalSiren)

        case .seasonEnd:
            hapticsManager.onChampionshipWin()

        default:
            hapticsManager.onButtonTap()
        }
    }
