...
class NotificationManager: ObservableObject {
    ...
    private func createAlertMessage(for alert: AlertFlag, player: EnhancedPlayer) -> String {
        switch alert.type {
        case .priceDrop:
            return "\(player.name) price has dropped \(abs(player.priceChange/1000))k! Consider buying."
        case .breakEvenCliff:
            return "\(player.name) is approaching breakeven cliff at \(player.breakeven)!"
        case .cashCowSell:
            return "Cash cow \(player.name) has generated \(player.cashGenerated/1000)k - consider selling!"
        case .injuryRisk:
            return "\(player.name) has elevated injury risk - monitor closely."
        case .roleChange:
            return "\(player.name) role change detected - review team impact."
        case .weatherRisk:
            return "Weather alert for \(player.name)'s match - potential scoring impact."
        case .contractYear:
            return "\(player.name) in contract year - motivation boost expected!"
        case .premiumBreakout:
            return "\(player.name) showing premium breakout potential!"
        }
    }
}
...
enum NotificationAction: String, CaseIterable {
    ...
    var title: String {
        switch self {
        case .viewPlayer: return "View Player"
        case .dismiss: return "Dismiss"
        case .makeChange: return "Make Change"
...
extension NotificationManager {
    ...
    private func createNotificationCategories() -> [UNNotificationCategory] {
        ...
        let playerAlertActions = [
            UNNotificationAction(identifier: NotificationAction.viewPlayer.rawValue,
                               title: NotificationAction.viewPlayer.title,
                               options: [.foreground]),
            UNNotificationAction(identifier: NotificationAction.dismiss.rawValue,
                               title: NotificationAction.dismiss.title,
                               options: [])
        ]
        categories.append(UNNotificationCategory(identifier: NotificationCategory.playerAlert.rawValue,
        ...
        let breakevenActions = [
            UNNotificationAction(identifier: NotificationAction.viewPlayer.rawValue,
                               title: NotificationAction.viewPlayer.title,
                               options: [.foreground]),
            UNNotificationAction(identifier: NotificationAction.makeChange.rawValue,
                               title: NotificationAction.makeChange.title,
                               options: [.foreground])
        ]
        categories.append(UNNotificationCategory(identifier: NotificationCategory.breakevenAlert.rawValue,
        ...
        categories.append(UNNotificationCategory(identifier: NotificationCategory.priceAlert.rawValue,
        ...
        categories.append(UNNotificationCategory(identifier: NotificationCategory.captainSuggestion.rawValue,
        ...
        let lockoutActions = [
            UNNotificationAction(identifier: NotificationAction.makeChange.rawValue,
                               title: "Review Team",
                               options: [.foreground]),
            UNNotificationAction(identifier: NotificationAction.remindLater.rawValue,
                               title: NotificationAction.remindLater.title,
                               options: [])
        ]
        categories.append(UNNotificationCategory(identifier: NotificationCategory.lockoutReminder.rawValue,
        ...
        categories.append(UNNotificationCategory(identifier: NotificationCategory.tradeSuggestion.rawValue,
