...
final class DashboardViewModel: ViewModel {
    ...
    @Published var selectedPosition: PlayerPosition?
    ...
    private func setupLiveUpdates() {
        ...
        liveStatsUseCase.livePlayersPublisher()
            .receive(on: DispatchQueue.main)
            .sink(
                receiveCompletion: { completion in
                    if case .failure(let error) = completion {
                        self.playersState = .error(error)
                    }
                },
                receiveValue: { players in
                    self.playersState = .loaded(players)
                }
            )
            .store(in: &cancellables)
        
        // Live summary updates
        liveStatsUseCase.liveSummaryPublisher()
            .receive(on: DispatchQueue.main)
            .sink(
                receiveCompletion: { completion in
                    if case .failure(let error) = completion {
                        self.summaryState = .error(error)
                    }
                },
...
extension DashboardViewModel {
    ...
    var availablePositions: [PlayerPosition] {
        return PlayerPosition.allCases
    }
}
