...
final class APITestViewModel: ObservableObject {
    ...
    func refreshData() async {
        ...
        async let playersTask: Void = loadPlayers()
        async let cashCowsTask: Void = loadCashCows()
        async let captainTask: Void = loadCaptainSuggestions()
        
        // Wait for all tasks to complete
        _ = await (healthTask, playersTask, cashCowsTask, captainTask)
    }
